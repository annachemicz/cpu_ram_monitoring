...
def _shopping_item_key(name, unit):
    return f"{name}||{unit}"


def _shift_plan_entries_to_today(plan_entries):
    if not plan_entries:
        return plan_entries

    try:
        entries = sorted(plan_entries, key=lambda e: e["date"])
        plan_start = date.fromisoformat(entries[0]["date"])
    ...
    for entry in entries:
        try:
            ...
            continue
        shifted.append({"date": (entry_date + delta).isoformat(), "slots": entry["slots"]})

    storage.save_plan(shifted)
    return shifted
...
def jadlospis():
    ...
    if plan_entries:
        plan_entries = _shift_plan_entries_to_today(plan_entries)
...
def lista_zakupow():
    ...
    profile = storage.load_profile()
    owned_items = set(profile.get("owned_shopping_items", []))
    hide_owned = request.args.get("hide_owned", "0") == "1"
    ...
    if hide_owned and owned_items:
        filtered = {}
        for category, items in shopping_list.items():
            visible = [item for item in items if _shopping_item_key(item["name"], item["unit"]) not in owned_items]
            if visible:
...
@bp.route("/lista-zakupow/aktualizuj", methods=["POST"])
def aktualizuj_lista_zakupow():
    profile = storage.load_profile()
    submitted = request.form.getlist("owned_item")
    hide_owned = request.form.get("hide_owned", "0") == "1"

    profile["owned_shopping_items"] = sorted(set(submitted))
    storage.save_profile(profile)
    flash("Zaktualizowano listę posiadanych produktów.", "success")
    return redirect(url_for("main.lista_zakupow", hide_owned="1" if hide_owned else None))
